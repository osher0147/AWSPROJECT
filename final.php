...
if (isset($_POST['pay'])) {
  ...
  if ($resultcmd->num_rows > 0) {
    while ($rowcmd = $resultcmd->fetch_assoc()) {
      ...
      $queryStock = "SELECT * FROM command INNER JOIN product ON command.id_produit = product.id WHERE `statut`='ordered'";
      // echo $queryStock;
      $stockResult = $connection->query($queryStock);
      if ($stockResult->num_rows > 0) {
        while ($rowcategory = $stockResult->fetch_assoc()) {
          $stockCount = $rowcategory['stock'];
          $quantity = $rowcategory['quantity'];
          $quant = $stockCount - $quantity;

          $id_produit = $rowcategory['id_produit'];
          $quantity = $rowcategory['quantity'];
          $queryStock2 = "UPDATE `product` SET `stock` = '$quant' WHERE `product`.`id` = $id_produit;";
          // echo $queryStock2;
          // $querypay = "UPDATE command SET statut = 'paid' WHERE id_user = '{$_SESSION['id']}' AND statut = 'ordered'";
          $resultpay = mysqli_query($connection, $queryStock2);
        }
      }
    }
  }
    ...
    <div class="col s12 m6">
      <div class="card center-align">
        ...
        <div class="card-content center-align">
          ...
          <p>Your order is on its way Dear : <h5 class="green-text"><?php echo "$firstname_sess" . " " . "$lastname_sess";  ?></h5>
          </p>
        </div>
      </div>
